# Trustee GPU appraisal policy for Cohere pod VMs.
#
# Generated by .github/actions/generate-policy from this template and the
# targets in attestation-policy/policy-manifest.yaml.
#
# Paired with the CPU policy. Trustee looks a policy up per TEE class the
# evidence carries, and a class with no policy fails the whole appraisal, so
# this file is mandatory for a GPU node even when only the CPU is of interest.
# The converse also holds and is easy to miss: a policy this strict is never
# consulted for a peer that presents no GPU evidence, so requiring that a peer
# have a GPU at all is the relying party's configuration, not anything written
# here.
#
...
# JWT. The same NRAS claim set the ITA outputs (claims 3.0 on /v4) but
# in a different envelope: flat under input.nvidia rather than nested per
...
# runs this same file against each, so nothing here iterates.
...
# One entry per distinct driver version across the manifest's pod VM images,
# each read from that image's own measurements.json. Images shipping the same
# driver collapse to one entry, so this is usually shorter than the image list.
#
# Generated rather than written inline so that a manifest with no GPU target
# renders an empty set that denies, instead of a fabricated constant that reads
# like a real check while comparing against a value no GPU reports.
