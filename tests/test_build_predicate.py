...
def run_build_predicate(
    ...
    firmware_ref: str = "ovmf-2024-08",
    uki_ref: str = "sha384:abc",
    baseline_ref: str = "commit:abc123",
...
class TestPredicateBuilder:
    ...
    def test_generates_predicates_for_all_models(self, artifacts_dir: Path):
        result = run_build_predicate(artifacts_dir)
    ...
    def test_predicate_has_correct_model_path(self, artifacts_dir: Path):
        run_build_predicate(artifacts_dir)
    ...
    def test_predicate_contains_measurements(self, artifacts_dir: Path, real_measurements: dict):
        run_build_predicate(artifacts_dir)
        pred = json.loads((artifacts_dir / "command-r-plus" / "predicate.json").read_text())
    ...
    def test_predicate_contains_policy_id(self, artifacts_dir: Path):
        policy_id = "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee"
        run_build_predicate(artifacts_dir, policy_id=policy_id)
        pred = json.loads((artifacts_dir / "command-r-plus" / "predicate.json").read_text())

        assert pred["policy_id"] == policy_id

    def test_predicate_contains_tool_versions(self, artifacts_dir: Path):
        run_build_predicate(
            artifacts_dir,
    ...
    def test_predicate_contains_source_artifacts(self, artifacts_dir: Path):
        run_build_predicate(
            artifacts_dir,
            firmware_ref="fw-v2",
            uki_ref="uki-v3",
            baseline_ref="bl-commit-xyz",
        )
        pred = json.loads((artifacts_dir / "command-r-plus" / "predicate.json").read_text())
    ...
    def test_predicate_chain_linking(self, artifacts_dir: Path):
        run_build_predicate(artifacts_dir, previous_log_index=42)
        pred = json.loads((artifacts_dir / "command-r-plus" / "predicate.json").read_text())

        assert pred["previous_rekor_log_index"] == 42

    def test_genesis_entry_has_zero_previous(self, artifacts_dir: Path):
        run_build_predicate(artifacts_dir, previous_log_index=0)
        pred = json.loads((artifacts_dir / "command-r-plus" / "predicate.json").read_text())

        assert pred["previous_rekor_log_index"] == 0

    def test_predicate_has_rego_hash(self, artifacts_dir: Path):
        run_build_predicate(artifacts_dir)
        pred = json.loads((artifacts_dir / "command-r-plus" / "predicate.json").read_text())
    ...
    def test_predicate_has_initdata_hash(self, artifacts_dir: Path):
        run_build_predicate(artifacts_dir)
        pred = json.loads((artifacts_dir / "command-r-plus" / "predicate.json").read_text())
    ...
    def test_predicate_has_iso_timestamp(self, artifacts_dir: Path):
        run_build_predicate(artifacts_dir)
        pred = json.loads((artifacts_dir / "command-r-plus" / "predicate.json").read_text())
    ...
    def test_predicate_event_type_is_activated(self, artifacts_dir: Path):
        run_build_predicate(artifacts_dir)
        pred = json.loads((artifacts_dir / "command-r-plus" / "predicate.json").read_text())
        assert pred["event_type"] == "policy_activated"

    def test_no_artifacts_fails(self, tmp_path: Path):
        empty = tmp_path / "empty"
        empty.mkdir()
        result = run_build_predicate(empty)
...
class TestPredicateSchemaValidation:
    ...
    def test_generated_predicate_validates_against_schema(
        self, artifacts_dir: Path, schema_path: Path
    ):
        run_build_predicate(artifacts_dir)
